...
import { default as cssTree } from "./vendor/css-tree.js";

const { parse: parse$3$1 } = cssTree;
...
class Stats {
	...
	stop(label) {
		...
		this.current_children = this.current_timing
			? this.current_timing.children
			: this.timings;
	}
	...
	walk(expression, {
		enter(node, parent) {
			if (node.type === "ImportDeclaration") {
				...
				/(Function(Declaration|Expression)|ArrowFunctionExpression)/.test(
					node.type
				)
			) {
...
function needs_parens(node, parent, is_right) {
	...
	if (is_right) {
		...
		return (
			OPERATOR_PRECEDENCE[node.operator] <= OPERATOR_PRECEDENCE[parent.operator]
		);
	}
...
const handlers = {
	...
	IfStatement(node, state) {
		...
		if (node.alternate) {
			chunks.push(c(" else "), ...handle(node.alternate, state));
		}
	...
	ReturnStatement(node, state) {
		if (node.argument) {
			return [c("return "), ...handle(node.argument, state), c(";")];
		} else {
			return [c("return;")];
		}
	},
	...
	ForStatement: scoped((node, state) => {
		...
		chunks.push(c(") "), ...handle(node.body, state));
	...
	ClassDeclaration(node, state) {
		...
		if (node.superClass) {
			chunks.push(c("extends "), ...handle(node.superClass, state), c(" "));
		}
	...
	ExportNamedDeclaration(node, state) {
		...
		if (node.declaration) {
			...
			if (node.source) {
				chunks.push(c(" from "), ...handle(node.source, state));
			}
		}
	...
	MethodDefinition(node, state) {
		...
		chunks.push(c(") "), ...handle(node.value.body, state));
	...
	ArrowFunctionExpression: scoped((node, state) => {
		...
		if (node.body.type === "ObjectExpression") {
			chunks.push(c("("), ...handle(node.body, state), c(")"));
		} else {
			chunks.push(...handle(node.body, state));
		}
	...
	UnaryExpression(node, state) {
		...
		if (
			...
			chunks.push(c("("), ...handle(node.argument, state), c(")"));
		} else {
	...
	BinaryExpression(node, state) {
		...
		if (needs_parens(node.left, node, false)) {
			chunks.push(c("("), ...handle(node.left, state), c(")"));
		} else {
			chunks.push(...handle(node.left, state));
		}
		...
		if (needs_parens(node.right, node, true)) {
			chunks.push(c("("), ...handle(node.right, state), c(")"));
		} else {
	...
	ConditionalExpression(node, state) {
		...
		if (
			...
			chunks.push(c("("), ...handle(node.test, state), c(")"));
		}
	...
	NewExpression(node, state) {
		...
		if (
			...
			chunks.push(c("("), ...handle(node.callee, state), c(")"));
		} else {
	...
	MemberExpression(node, state) {
		...
		if (
			...
			chunks.push(c("("), ...handle(node.object, state), c(")"));
		} else {
		...
		if (node.computed) {
			...
			chunks.push(c("["), ...handle(node.property, state), c("]"));
		} else {
			chunks.push(
				c(node.optional ? "?." : "."),
	...
	Literal(node, state) {
		if (typeof node.value === "string") {
			return [
				...
				c(
					JSON.stringify(node.value).replace(re, (_m, _i, at, hash, name) => {
						if (at) return "@" + name;
						if (hash) return "#" + name;
	...
	walk(node, {
		...
		leave(node, parent, key, index) {
			...
			if (node.type === "Literal") {
				if (typeof node.value === "string") {
					re.lastIndex = 0;
					node.value = node.value.replace(re, (m, i) =>
						+i in values ? values[+i] : m
					);
				}
			}
			...
			if (node.type === "TemplateElement") {
				re.lastIndex = 0;
				node.value.raw = node.value.raw.replace(re, (m, i) =>
					+i in values ? values[+i] : m
				);
			}
...
const parse = (source, opts) => {
	...
	const ast = parse$3(source, { onComment, ...opts });
...
function read_script(parser, start, attributes) {
	...
	const source =
		parser.template.slice(0, script_start).replace(/[^\n]/g, " ") +
...
class FuzzySet {
	...
	constructor(arr) {
		FuzzySet.prototype.__init.call(this);
		FuzzySet.prototype.__init2.call(this);
		FuzzySet.prototype.__init3.call(this);
		// initialization
...
function tag(parser) {
	...
	if (is_closing_tag) {
		...
		while (parent.name !== name) {
			if (parent.type !== "Element") {
				const message =
					...
					parser.last_auto_closed_tag.tag === name
		...
		if (
			parser.last_auto_closed_tag &&
			parser.stack.length < parser.last_auto_closed_tag.depth
		) {
...
function read_attribute(parser, unique_names) {
	...
	const type =
		colon_index !== -1 && get_directive_type(name.slice(0, colon_index));
	...
	if (type) {
		const [directive_name, ...modifiers] = name
			.slice(colon_index + 1)
			.split("|");
...
function read_context(parser) {
	...
	try {
		...
		space_with_newline =
			space_with_newline.slice(0, first_space) +
			space_with_newline.slice(first_space + 1);
...
class Parser {
	...
	constructor(template, options) {
		...
		Parser.prototype.__init5.call(this);
	...
	eat(str, required, message) {
		...
		if (required) {
			this.error({
				code: `unexpected-${
					this.index === this.template.length ? "eof" : "token"
				}`,
...
class Block {
	...
	constructor(options) {
		Block.prototype.__init.call(this);
		Block.prototype.__init2.call(this);
		Block.prototype.__init3.call(this);
		Block.prototype.__init4.call(this);
		Block.prototype.__init5.call(this);
...
class AwaitBlockBranch extends Wrapper {
	...
	constructor(
		...
		AwaitBlockBranch.prototype.__init.call(this);
		this.status = status;
		...
		this.add_context(
			parent.node[status + "_node"],
			parent.node[status + "_contexts"]
		);
	...
	render_destructure() {
		const props = this.value_contexts.map(
			(prop) =>
				b`#ctx[${
					this.block.renderer.context_lookup.get(prop.key.name).index
				}] = ${prop.modifier(x`#ctx[${this.value_index}]`)};`
		);
		const get_context = this.block.renderer.component.get_unique_name(
			`get_${this.status}_context`
		);
...
class AwaitBlockWrapper extends Wrapper {
	...
	constructor(renderer, block, parent, node, strip_whitespace, next_sibling) {
		super(renderer, block, parent, node);
		AwaitBlockWrapper.prototype.__init2.call(this);
	...
	render(block, target) {
		...
		if (opts.length) {
			if (opts.length === 1 && opts[0] === "capture") {
				...
				args.push(
					x`{ ${opts.map((opt) =>
						opt === "nonpassive" ? p`passive: false` : p`${opt}: true`
					)} }`
				);
			}
...
class BodyWrapper extends Wrapper {
	constructor(renderer, block, parent, node) {
		...
		this.handlers = this.node.handlers.map(
			(handler) => new EventHandlerWrapper(handler, this)
		);
	}
...
class ElseBlockWrapper extends Wrapper {
	...
	constructor(renderer, block, parent, node, strip_whitespace, next_sibling) {
		super(renderer, block, parent, node);
		ElseBlockWrapper.prototype.__init.call(this);
		this.block = block.child({
			comment: create_debugging_comment(node, this.renderer.component),
...
class EachBlockWrapper extends Wrapper {
	...
	constructor(renderer, block, parent, node, strip_whitespace, next_sibling) {
		super(renderer, block, parent, node);
		EachBlockWrapper.prototype.__init2.call(this);
		EachBlockWrapper.prototype.__init3.call(this);
		this.cannot_use_innerhtml();
		this.not_static_content();
		...
		const fixed_length =
			...
			node.expression.node.elements.every(
		...
		const each_block_value = renderer.component.get_unique_name(
			`${this.var.name}_value`
		);
		...
		this.vars = {
			...
			each_block_value,
			get_each_context: renderer.component.get_unique_name(
				`get_${this.var.name}_context`
			),
			iterations,
			...
			fixed_length,
			data_length:
				fixed_length === null ? x`${each_block_value}.${length}` : fixed_length,
			view_length:
				fixed_length === null ? x`${iterations}.length` : fixed_length,
		};

		const object = get_object(node.expression.node);
		const store =
			object.type === "Identifier" && object.name[0] === "$"
				? object.name.slice(1)
				: null;
	...
	render(block, parent_node, parent_nodes) {
		...
		if (this.else) {
			const each_block_else = component.get_unique_name(
				`${this.var.name}_else`
			);
		...
		this.context_props = this.node.contexts.map(
			(prop) =>
				...
				}] = ${prop.modifier(x`list[i]`)};`
		);

		if (this.node.has_binding)
			this.context_props.push(
				b`child_ctx[${
					renderer.context_lookup.get(this.vars.each_block_value.name).index
				}] = list;`
			);
		if (this.node.has_binding || this.node.has_index_binding || this.node.index)
			this.context_props.push(
				b`child_ctx[${
					renderer.context_lookup.get(this.index_name.name).index
				}] = i;`
			);
	...
	render_keyed({
		...
		if (this.dependencies.size) {
			...
			this.updates.push(b`
				...
				${iterations} = @update_keyed_each(${iterations}, #dirty, ${get_key}, ${
				dynamic ? 1 : 0
			}, #ctx, ${
				this.vars.each_block_value
		...
		if (this.dependencies.size) {
			...
			const update = b`
				${
					...
					b`const #old_length = ${this.vars.each_block_value}.length;`
				}
				${this.vars.each_block_value} = ${snippet};
				${
					this.renderer.options.dev &&
				...
				for (#i = ${start}; #i < ${data_length}; #i += 1) {
					const child_ctx = ${this.vars.get_each_context}(#ctx, ${
				this.vars.each_block_value
			}, #i);
		...
		switch (this.node.name) {
			case "group": {
				const {
					...
					index,
					keypath,
				} = get_binding_group(parent.renderer, this.node, block);

				block.renderer.add_to_context("$$binding_groups");

				if (is_context && !block.binding_group_initialised.has(keypath)) {
					if (contexts.length > 1) {
						let binding_group = x`${block.renderer.reference(
							"$$binding_groups"
						)}[${index}]`;
						for (const name of contexts.slice(0, -1)) {
							binding_group = x`${binding_group}[${block.renderer.reference(
								name
							)}]`;
							block.chunks.init.push(
				...
				block.chunks.destroy.push(
					b`${binding_group(true)}.splice(${binding_group(true)}.indexOf(${
						parent.var
					}), 1);`
				);
				break;
		...
		renderer.binding_groups.set(keypath, {
			binding_group: (to_reference = false) => {
				...
				if (to_reference) {
					...
					_secondary_indexes = _secondary_indexes.map((name) =>
...
function get_event_handler(binding, renderer, block, name, lhs) {
	...
	const value = get_value_from_dom(
		renderer,
		binding.parent,
		binding,
		block,
...
function get_value_from_dom(
	...
	if (name === "group") {
		if (type === "checkbox") {
			const { binding_group, contexts } = get_binding_group(
				renderer,
				binding.node,
				block
			);
...
function add_action(block, target, action) {
	...
	if (properties.length) {
		const member_expression = properties.reduce(
			(lhs, rhs) => x`${lhs}.${rhs}`,
			fn
		);
		block.event_listeners.push(
	...
	if (contextual_dependencies.size) {
		...
		for (let id of params) {
			...
			if (block.variables.has(id.name)) {
				...
				for (
					let i = 1;
					block.variables.has(alias) &&
					!compare_node(block.variables.get(alias).init, value);
					alias = `${id.name}_${i++}`
				);
		...
		const condition = Array.from(args)
			.map(
				(name) =>
					x`${name} !== ${block.renderer.reference(
						alias_map.get(name.name) || name.name
					)}`
			)
			.reduce((lhs, rhs) => x`${lhs} || ${rhs}`);
		...
		block.chunks.update.push(b`
			if (${condition}) {
				...
				${args.map(
					(a) =>
						b`${a} = ${block.renderer.reference(
							alias_map.get(a.name) || a.name
						)}`
				)};
				${assign}();
			}`);
...
function invalidate(
	...
	const extra_args = tail
		.map((variable) => get_invalidated(variable))
		.filter(Boolean);

	if (is_store_value) {
		return x`@set_store_value(${head.name.slice(1)}, ${node}, ${
			head.name
		}, ${extra_args})`;
	}
...
function renderer_invalidate(
	...
	const filtered = Array.from(deps).filter((n) =>
		renderer.context_lookup.has(n)
	);
...
class Expression {
	...
	constructor(component, owner, template_scope, info, lazy) {
		Expression.prototype.__init.call(this);
		Expression.prototype.__init2.call(this);
		Expression.prototype.__init3.call(this);
		Expression.prototype.__init4.call(this);
		Expression.prototype.__init5.call(this);
		Expression.prototype.__init6.call(this);
					...
					if (dependencies.size === 0 && contextual_dependencies.size === 0) {
						...
						if (owner.type === "Attribute" && owner.parent.name === "slot") {
							const dep_scopes = new Set(
								deps.map((name) => template_scope.get_owner(name))
							);
...
class Tag extends Wrapper {
	...
	is_dependencies_static() {
		return (
			this.node.expression.contextual_dependencies.size === 0 &&
			this.node.expression.dynamic_dependencies().length === 0
		);
	}
	...
	rename_this_method(block, update) {
		...
		const value =
			this.node.should_cache && block.get_unique_name(`${this.var.name}_value`);
...
class MustacheTagWrapper extends Tag {
	...
	constructor(renderer, block, parent, node) {
		super(renderer, block, parent, node);
		MustacheTagWrapper.prototype.__init.call(this);
	}
...
class RawMustacheTagWrapper extends Tag {
	...
	constructor(renderer, block, parent, node) {
		super(renderer, block, parent, node);
		RawMustacheTagWrapper.prototype.__init.call(this);
		this.cannot_use_innerhtml();
		this.not_static_content();
	}
	...
	render(block, parent_node, _parent_nodes) {
		...
		if (can_use_innerhtml) {
			...
			const update_anchor = needs_anchor
				? html_anchor
				: this.next
				? this.next.var
				: "null";
			...
			block.chunks.hydrate.push(
				b`${html_tag} = new @HtmlTag(${update_anchor});`
			);
			block.chunks.mount.push(
				b`${html_tag}.m(${init}, ${parent_node || "#target"}, ${
	...
	if (owner && owner.node.type === "InlineComponent") {
		...
		if (!owner.slots.has(name)) {
			const child_block = block.child({
				comment: create_debugging_comment(
					element.node,
...
const events = [
	{
		...
		filter: (node, _name) =>
			node.name === "textarea" ||
			(node.name === "input" &&
				!/radio|checkbox|range|file/.test(
	...
	},
	{
		...
		filter: (node, _name) =>
			node.name === "select" ||
			(node.name === "input" &&
				/radio|checkbox|file/.test(node.get_static_attribute_value("type"))),
	},
	{
		event_names: ["change", "input"],
		filter: (node, _name) =>
			node.name === "input" &&
			node.get_static_attribute_value("type") === "range",
	},
...
class ElementWrapper extends Wrapper {
	constructor(renderer, block, parent, node, strip_whitespace, next_sibling) {
		...
		this.bindings = this.node.bindings.map(
			(binding) => new BindingWrapper(block, binding, this)
		);
		...
		[node.animation, node.outro, ...node.actions, ...node.classes].forEach(
			(directive) => {
	...
	render(block, parent_node, parent_nodes) {
		...
		const node = this.var;
		const nodes =
			parent_nodes && block.get_unique_name(`${this.var.name}_nodes`); // if we're in unclaimable territory, i.e. <head>, parent_nodes is null
		...
		if (
			...
			this.fragment.nodes.length > 0
		) {
			if (
				this.fragment.nodes.length === 1 &&
				this.fragment.nodes[0].node.type === "Text"
			) {
				...
				block.chunks.create.push(
					b`${node}.${
						this.can_use_innerhtml ? "innerHTML" : "textContent"
					} = ${literal};`
				);
			}
		} else {
		...
		if (renderer.options.dev) {
			...
			block.chunks.hydrate.push(
				b`@add_location(${this.var}, ${renderer.file_var}, ${loc.line - 1}, ${
					loc.column
				}, ${this.node.start});`
			);
		}
	...
	can_use_textcontent() {
		return (
			...
			this.fragment.nodes.every(
				(node) => node.node.type === "Text" || node.node.type === "MustacheTag"
			)
		);
	}
	...
	add_bindings(block, binding_group) {
		...
		const needs_lock = binding_group.bindings.some(
			(binding) => binding.needs_lock
		);
		...
		binding_group.bindings.forEach((binding) => {
			...
			add_to_set(
				contextual_dependencies,
				binding.handler.contextual_dependencies
			);
		...
		if (binding_group.events[0] === "timeupdate") {
			animation_frame = block.get_unique_name(
				`${this.var.name}_animationframe`
			);
		...
		binding_group.events.forEach((name) => {
			if (name === "elementresize") {
				// special case
				const resize_listener = block.get_unique_name(
					`${this.var.name}_resize_listener`
				);
		...
		if (should_initialise) {
			const callback = has_local_function
				? handler
				: x`() => ${callee}.call(${this.var})`;
			block.chunks.hydrate.push(
	...
	add_this_binding(block, this_binding) {
		...
		const binding_callback = bind_this(
			renderer.component,
			block,
			this_binding,
			this.var
	...
	add_spread_attributes(block) {
		...
		this.attributes.forEach((attr) => {
			...
			const condition =
				dependencies.length > 0 ? block.renderer.dirty(dependencies) : null;
			...
			if (attr instanceof SpreadAttributeWrapper) {
				...
				initial_props.push(
					x`{ ${name}: ${attr.get_init(block, attr.get_value(block))} }`
				);
				const snippet = x`{ ${name}: ${
					attr.should_cache ? attr.last : attr.get_value(block)
				} }`;

				updates.push(
					condition
						? x`${attr.get_dom_update_conditions(
								block,
								condition
						  )} && ${snippet}`
						: snippet
	...
	add_transitions(block) {
		...
		if (intro === outro) {
			...
			const intro_name =
				intro && block.get_unique_name(`${this.var.name}_intro`);
			const outro_name =
				outro && block.get_unique_name(`${this.var.name}_outro`);
			...
			if (intro) {
				...
				if (outro) {
					...
					block.chunks.outro.push(
						b`if (${intro_name}) ${intro_name}.invalidate();`
					);
				} else {
					intro_block = b`
						if (!${intro_name}) {
			...
			if (outro) {
				...
				block.chunks.destroy.push(
					b`if (detaching && ${outro_name}) ${outro_name}.end();`
				);
			}
		}
	...
	add_classes(block) {
		...
		this.node.classes.forEach((class_directive) => {
			...
			if (has_spread) {
				...
				this.class_dependencies.length
			) {
				const all_dependencies = this.class_dependencies.concat(
					...dependencies
				);
...
function to_html(wrappers, block, literal, state, can_use_raw_text) {
	wrappers.forEach((wrapper) => {
		if (wrapper instanceof TextWrapper) {
			...
			wrapper instanceof MustacheTagWrapper ||
			wrapper instanceof RawMustacheTagWrapper
		) {
			literal.quasis.push(state.quasi);
...
class HeadWrapper extends Wrapper {
	...
	render(block, _parent_node, _parent_nodes) {
		...
		if (this.renderer.options.hydratable && this.fragment.nodes.length) {
			...
			block.chunks.claim.push(
				b`const ${nodes} = @query_selector_all('[data-svelte="${this.node.id}"]', @_document.head);`
			);
		}
...
class IfBlockBranch extends Wrapper {
	...
	constructor(renderer, block, parent, node, strip_whitespace, next_sibling) {
		super(renderer, block, parent, node);
		IfBlockBranch.prototype.__init.call(this);
...
class IfBlockWrapper extends Wrapper {
	...
	constructor(renderer, block, parent, node, strip_whitespace, next_sibling) {
		super(renderer, block, parent, node);
		IfBlockWrapper.prototype.__init2.call(this);
		IfBlockWrapper.prototype.__init3.call(this);
		this.cannot_use_innerhtml();
		this.not_static_content();
	...
	render_compound(
		...
		const select_block_type = this.renderer.component.get_unique_name(
			"select_block_type"
		);
				...
				function ${select_block_type}(#ctx, #dirty) {
					...
					${
						...
						(dependencies.length > 0
							? b`if (${condition} == null || ${block.renderer.dirty(
									dependencies
							  )}) ${condition} = !!${snippet}`
		...
		if (this.needs_update) {
			...
			const change_block = b`
				${
					if_exists_condition
						? b`if (${if_exists_condition}) ${name}.d(1)`
						: b`${name}.d(1)`
				};
				${name} = ${get_block};
				if (${name}) {
			...
			if (if_exists_condition) {
				block.chunks.update.push(
					b`if (${if_exists_condition}) ${name}.p(#ctx, #dirty);`
				);
			} else {
	...
	render_compound_with_outros(
		...
		const current_block_type_index = block.get_unique_name(
			...
			if (dynamic) {
				block.chunks.update.push(b`
					if (${current_block_type_index} === ${previous_block_index}) {
						${if_current_block_type_index(
							b`${if_blocks}[${current_block_type_index}].p(#ctx, #dirty);`
						)}
					} else {
						${change_block}
					}
		...
		block.chunks.destroy.push(
			if_current_block_type_index(
				b`${if_blocks}[${current_block_type_index}].d(${detaching});`
			)
		);
	}
...
class KeyBlockWrapper extends Wrapper {
	...
	constructor(renderer, block, parent, node, strip_whitespace, next_sibling) {
		super(renderer, block, parent, node);
		KeyBlockWrapper.prototype.__init.call(this);
		this.cannot_use_innerhtml();
		this.not_static_content();
...
class InlineComponentWrapper extends Wrapper {
	...
	constructor(renderer, block, parent, node, strip_whitespace, next_sibling) {
		super(renderer, block, parent, node);
		InlineComponentWrapper.prototype.__init.call(this);
		this.cannot_use_innerhtml();
		this.not_static_content();
		...
		if (this.node.children.length) {
			...
			this.slots.set(
				"default",
				get_slot_definition(default_slot, this.node.scope, this.node.lets)
			);
			this.fragment = new FragmentWrapper(
				renderer,
				default_slot,
	...
	render(block, parent_node, parent_nodes) {
			...
			this.slots.size > 0
				? [
					...
					${Array.from(this.slots).map(([name, slot]) => {
						return p`${name}: [${slot.block.name}, ${
							slot.get_context || null
		...
		const attribute_object = uses_spread
			...
			: x`{
				${this.node.attributes.map(
					(attr) => p`${attr.name}: ${attr.get_value(block)}`
				)},
				${initial_props}
			}`;
		...
		const dynamic_attributes = this.node.attributes.filter(
			(a) => a.get_dependencies().length > 0
		);
			...
			if (uses_spread) {
				...
				dynamic_attributes.forEach((attribute) => {
					...
					if (dependencies.length > 0) {
						...
						updates.push(b`
							if (${condition}) ${name_changes}.${attribute.name} = ${attribute.get_value(
							block
						)};
						`);
					}
		...
		const munged_bindings = this.node.bindings.map((binding) => {
			...
			if (binding.name === "this") {
				return bind_this(
					component,
					block,
					new BindingWrapper(block, binding, this),
					this.var
				);
			}

			const id = component.get_unique_name(
				`${this.var.name}_${binding.name}_binding`
			);
			...
			updates.push(b`
				if (!${updating} && ${renderer.dirty(
				Array.from(binding.expression.dependencies)
			)}) {
					${updating} = true;
			...
			const contextual_dependencies = Array.from(
		...
		if (this.node.name === "svelte:component") {
			...
			block.chunks.mount.push(b`
				if (${name}) {
					@mount_component(${name}, ${parent_node || "#target"}, ${
				parent_node ? "null" : "#anchor"
			});
				}
			`);

			const anchor = this.get_or_create_anchor(
				block,
				parent_node,
				parent_nodes
			);
			...
			block.chunks.destroy.push(
				b`if (${name}) @destroy_component(${name}, ${
					parent_node ? null : "detaching"
				});`
			);
		} else {
			const expression =
				this.node.name === "svelte:self"
					? component.name
					: this.renderer.reference(
							string_to_member_expression(this.node.name)
					  );
			...
			block.chunks.mount.push(
				b`@mount_component(${name}, ${parent_node || "#target"}, ${
					parent_node ? "null" : "#anchor"
				});`
			);
...
function get_spread_value(block, attribute) {
	return block
		? attribute.expression.manipulate(block)
...
class SlotWrapper extends Wrapper {
	...
	constructor(renderer, block, parent, node, strip_whitespace, next_sibling) {
		super(renderer, block, parent, node);
		SlotWrapper.prototype.__init.call(this);
		SlotWrapper.prototype.__init2.call(this);
		SlotWrapper.prototype.__init3.call(this);
		this.cannot_use_innerhtml();
		this.not_static_content();
	...
	render(block, parent_node, parent_nodes) {
			...
			this.node.values.forEach((attribute) => {
				if (attribute.type === "Spread") {
					add_to_set(
						...
						Array.from(attribute.dependencies).filter((name) =>
							this.is_dependency_dynamic(name)
						)
					);
				} else {
					const dynamic_dependencies = Array.from(
						attribute.dependencies
					).filter((name) => this.is_dependency_dynamic(name));
		...
		block.chunks.init.push(b`
			...
			${
				has_fallback
					? b`const ${slot_or_fallback} = ${slot} || ${this.fallback.name}(#ctx);`
					: null
			}
		`);
		...
		block.chunks.mount.push(b`
			if (${slot_or_fallback}) {
				${slot_or_fallback}.m(${parent_node || "#target"}, ${
			parent_node ? "null" : "#anchor"
		});
			}
		`);
		...
		const slot_update = get_slot_spread_changes_fn
			...
			if (${slot}.p && ${renderer.dirty(dynamic_dependencies)}) {
				@update_slot_spread(${slot}, ${slot_definition}, #ctx, ${renderer.reference(
					"$$scope"
		...
		const fallback_update =
			...
			b`
			if (${slot_or_fallback} && ${slot_or_fallback}.p && ${renderer.dirty(
				fallback_dynamic_dependencies
			)}) {
...
class TitleWrapper extends Wrapper {
	...
	render(block, _parent_node, _parent_nodes) {
		const is_dynamic = !!this.node.children.find(
			(node) => node.type !== "Text"
		);
		...
		Object.keys(events).forEach((event) => {
			...
			if (event === "scroll") {
				...
				renderer.meta_bindings.push(b`
					if (${condition}) {
						@_scrollTo(${scrollX || "@_window.pageXOffset"}, ${
...
function trimmable_at(child, next_sibling) {
	...
	return (
		next_sibling.node.find_nearest(/EachBlock/) ===
			child.find_nearest(/EachBlock/) ||
...
class Renderer {
	...
	constructor(component, options) {
		Renderer.prototype.__init.call(this);
		Renderer.prototype.__init2.call(this);
		Renderer.prototype.__init3.call(this);
		Renderer.prototype.__init4.call(this);
		Renderer.prototype.__init5.call(this);
		Renderer.prototype.__init6.call(this);
		Renderer.prototype.__init7.call(this);
		...
		this.context.forEach((member) => {
			...
			if (variable) {
				...
				if (
					variable.is_reactive_dependency &&
					(variable.mutated || variable.reassigned)
				)
					member.priority += 16;
		...
		this.context.sort(
			(a, b) => b.priority - a.priority || a.index.value - b.index.value
		);
...
function buildSourceMapTree(input, loader, relativeRoot) {
	...
	const children = sources.map((sourceFile, i) => {
		...
		const uri = resolve$1(
			sourceFile || "",
			resolve$1(sourceRoot || "", stripFilename(relativeRoot))
		);
...
class MappedCode {
	...
	concat(other) {
		...
		const [
			...
			sources_changed,
			sources_idx_changed,
		] = merge_tables(m1.sources, m2.sources);
		const [
			names,
			new_name_idx,
			names_changed,
			names_idx_changed,
		] = merge_tables(m1.names, m2.names);
...
function combine_sourcemaps(filename, sourcemap_list) {
	...
	let map_idx = 1;
	const map =
		sourcemap_list.slice(0, -1).find((m) => m.sources.length !== 1) ===
		undefined
			? remapping(
					// use array interface
...
// browser vs node.js
const b64enc =
	typeof btoa == "function" ? btoa : (b) => Buffer.from(b).toString("base64");
const b64dec =
	typeof atob == "function" ? atob : (a) => Buffer.from(a, "base64").toString();
...
function parse_attached_sourcemap(processed, tag_name) {
	...
	function log_warning(message) {
		...
		const code_start =
			processed.code.length < 100
				? processed.code
				: processed.code.slice(0, 100) + " [...]";
		console.warn(
			`warning: ${message}. processed.code = ${JSON.stringify(code_start)}`
		);
	}
	processed.code = processed.code.replace(regex, (_, match1, match2) => {
		const map_url = tag_name == "script" ? match1 || match2 : match1;
		const map_data = (map_url.match(
			/data:(?:application|text)\/json;(?:charset[:=]\S+?;)?base64,(\S*)/
		) || [])[1];
		if (map_data) {
...
function dom(component, options) {
	...
	const rest = uses_rest
		? b`
		const ${omit_props_names.name} = [${props
	...
	const set =
		...
		component.slots.size > 0
			...
			${$$props} => {
				...
				${
					uses_rest &&
					renderer.invalidate("$$restProps", x`$$restProps = ${compute_rest}`)
				}
				${writable_props.map(
					(prop) =>
			...
			inject_state = x`
				${$$props} => {
					${
						...
						renderer.invalidate(
							"$$props",
							x`$$props = @assign(@assign({}, $$props), $$new_props)`
						)
					}
					${injectable_vars.map(
						(v) =>
	...
	const reactive_store_subscriptions = reactive_stores
		...
		.map(
			...
			${
				component.compile_options.dev &&
				b`@validate_store(${name.slice(1)}, '${name.slice(1)}');`
			}
	...
	if (has_definition) {
		...
		body.push(b`
			function ${definition}(${args}) {
				...
				${
					component.compile_options.dev &&
					b`@validate_slots('${component.tag}', #slots, [${[
				...
				${
					...
					b`const $$binding_groups = [${[...renderer.binding_groups.keys()].map(
						(_) => x`[]`
					)}];`
				}
		...
		const declaration = b`
			class ${name} extends @SvelteElement {
				constructor(options) {
					...
					@init(this, { target: this.shadowRoot, props: ${init_props} }, ${definition}, ${
		...
		const declaration = b`
			class ${name} extends ${superclass} {
				constructor(options) {
					...
					${
						should_add_css &&
						b`if (!@_document.getElementById("${component.stylesheet.id}-style")) ${add_css}();`
					}
...
function DebugTag(node, renderer, options) {
	...
	renderer.add_expression(
		x`@debug(${filename ? x`"${filename}"` : x`null`}, ${
			line - 1
		}, ${column}, ${obj})`
	);
}
	...
	if (node.attributes.some((attr) => attr.is_spread)) {
		...
		node.attributes.forEach((attribute) => {
			if (attribute.is_spread) {
				...
				if (name === "value" && node.name.toLowerCase() === "textarea") {
					...
					args.push(
						x`{ ${attribute.name}: ${attribute.chunks[0].node} || null }`
					);
				} else {
					args.push(
		...
		node.attributes.forEach((attribute) => {
			...
			if (name === "value" && node.name.toLowerCase() === "textarea") {
				...
				renderer.add_string(" ");
				renderer.add_expression(
					x`${attribute.chunks[0].node} ? "${attribute.name}" : ""`
				);
...
class Renderer$1 {
	...
	constructor({ name }) {
		Renderer$1.prototype.__init.call(this);
		Renderer$1.prototype.__init2.call(this);
		Renderer$1.prototype.__init3.call(this);
		this.name = name;
		this.push();
	}
...
function create_module(
	...
	throw new Error(
		`options.format is invalid (must be ${list(Object.keys(wrappers$1))})`
	);
}
...
function cjs(
	...
	const exports = module_exports.map(
		(x) =>
			b`exports.${{ type: "Identifier", name: x.as }} = ${{
				type: "Identifier",
				name: x.name,
			}};`
	);
...
(function (BlockAppliesToNode) {
	const NotPossible = 0;
	BlockAppliesToNode[(BlockAppliesToNode["NotPossible"] = NotPossible)] =
		"NotPossible";
	const Possible = NotPossible + 1;
	BlockAppliesToNode[(BlockAppliesToNode["Possible"] = Possible)] = "Possible";
	const UnknownSelectorType = Possible + 1;
	BlockAppliesToNode[
		(BlockAppliesToNode["UnknownSelectorType"] = UnknownSelectorType)
	] = "UnknownSelectorType";
})(BlockAppliesToNode || (BlockAppliesToNode = {}));
var NodeExist;
(function (NodeExist) {
	const Probably = 1;
	NodeExist[(NodeExist["Probably"] = Probably)] = "Probably";
	const Definitely = 2;
	NodeExist[(NodeExist["Definitely"] = Definitely)] = "Definitely";
})(NodeExist || (NodeExist = {}));

const whitelist_attribute_selector = new Map([["details", new Set(["open"])]]);
			...
			while (i-- > 1) {
				...
				if (
					...
					component.error(selector, {
						...
						message:
							":global(...) must be the first element in a compound selector",
					});
				}
			}
		});
		...
		if (block.combinator.type === "WhiteSpace") {
			for (const ancestor_block of blocks) {
				...
				while ((parent = get_element_parent(parent))) {
					if (
						block_might_apply_to_node(ancestor_block, parent) !==
		...
		if (prev.type === "Element") {
			if (
				!prev.attributes.find(
					(attr) =>
						attr.type === "Attribute" && attr.name.toLowerCase() === "slot"
				)
			) {
...
function get_possible_last_child(block, adjacent_only) {
	...
	if (block.type === "EachBlock") {
		...
		const not_exhaustive =
			!has_definite_elements(if_result) || !has_definite_elements(else_result);
...
class Rule {
	constructor(node, stylesheet, parent) {
		this.node = node;
		this.parent = parent;
		this.selectors = node.selector.children.map(
			(node) => new Selector(node, stylesheet)
		);
	...
	is_used(dev) {
		if (
			this.parent &&
			this.parent.node.type === "Atrule" &&
			is_keyframes_node(this.parent.node)
		)
			return true;
	...
	get_max_amount_class_specificity_increased() {
		return Math.max(
			...this.selectors.map((selector) =>
				selector.get_amount_class_specificity_increased()
			)
		);
	}
}
...
class Declaration {
	...
	transform(code, keyframes) {
		const property =
			this.node.property && remove_css_prefix(this.node.property.toLowerCase());
...
class Atrule {
	...
	get_max_amount_class_specificity_increased() {
		return Math.max(
			...this.children.map((rule) =>
				rule.get_max_amount_class_specificity_increased()
			)
		);
	}
}
...
class Stylesheet {
	...
	constructor(source, ast, filename, dev) {
		Stylesheet.prototype.__init.call(this);
		Stylesheet.prototype.__init2.call(this);
		Stylesheet.prototype.__init3.call(this);
		this.source = source;
		this.ast = ast;
		this.filename = filename;
		this.dev = dev;
						...
						if (is_keyframes_node(node)) {
							node.expression.children.forEach((expression) => {
								if (
									...
									!expression.name.startsWith("-global-")
								) {
									this.keyframes.set(
										expression.name,
										`${this.id}-${expression.name}`
									);
								}
							});
		...
		node.elements.forEach((element, i) => {
			if (element && element.type === "RestElement") {
				...
				unpack_destructuring(
					...
					element.left,
					(node) =>
						x`${modifier(node)}[${i}] !== undefined ? ${modifier(
							node
						)}[${i}] : ${element.right}`
				);
			} else {
			...
			if (property.type === "RestElement") {
				...
				if (value.type === "AssignmentPattern") {
					unpack_destructuring(
						...
						value.left,
						(node) =>
							x`${modifier(node)}.${key.name} !== undefined ? ${modifier(
								node
...
class EventHandler extends Node {
	...
	constructor(component, parent, template_scope, info) {
		...
		EventHandler.prototype.__init2.call(this);
		this.name = info.name;
		...
		if (info.expression) {
			...
			this.handler_name = component.get_unique_name(
				`${sanitize(this.name)}_handler`
			);
		}
	}
...
class EachBlock$1 extends AbstractBlock {
	...
	constructor(component, parent, scope, info) {
		super(component, parent, scope, info);
		EachBlock$1.prototype.__init.call(this);
		EachBlock$1.prototype.__init2.call(this);
		this.expression = new Expression(component, this, scope, info.expression);
...
class Attribute extends Node {
	...
	should_cache() {
		return this.is_static
			...
			? // @ts-ignore todo: probably error
			  this.chunks[0].node.type !== "Identifier" ||
			  this.scope.names.has(this.chunks[0].node.name)
			: true;
	}
}
...
class Binding extends Node {
	...
	constructor(component, parent, scope, info) {
		...
		if (
			...
			info.expression.type !== "MemberExpression"
		) {
		...
		this.is_readonly =
			...
			(isElement(parent) &&
				((parent.is_media_node() &&
					read_only_media_attributes.has(this.name)) ||
...
class Text$1 extends Node {
	...
	should_skip() {
		...
		const parent_element = this.find_nearest(
		...
		if (parent_element.type === "InlineComponent")
			return (
				...
				this === parent_element.children[0]
			);
		...
		if (/svg$/.test(parent_element.namespace)) {
			if (
				this.prev &&
				this.prev.type === "Element" &&
				this.prev.name === "tspan"
			)
				return false;
		}
...
class Let extends Node {
	...
	constructor(component, parent, scope, info) {
		super(component, parent, scope, info);
		Let.prototype.__init.call(this);
				...
				enter(node) {
					if (!applicable.has(node.type)) {
						component.error(node, {
							code: "invalid-let",
							message:
								"let directive value must be an identifier or an object/array pattern",
						});
					}
...
const a11y_no_onchange = new Set(["select", "option"]);
...
function get_namespace(parent, element, explicit_namespace) {
	...
	if (!parent_element) {
		return (
			explicit_namespace || (svg$1.test(element.name) ? namespaces.svg : null)
		);
	}
...
class Element$1 extends Node {
	...
	constructor(component, parent, scope, info) {
		super(component, parent, scope, info);
		Element$1.prototype.__init.call(this);
		Element$1.prototype.__init2.call(this);
		Element$1.prototype.__init3.call(this);
		Element$1.prototype.__init4.call(this);
		Element$1.prototype.__init5.call(this);
		Element$1.prototype.__init6.call(this);
		Element$1.prototype.__init7.call(this);
		Element$1.prototype.__init8.call(this);
		Element$1.prototype.__init9.call(this);
		this.name = info.name;
	...
	validate_attributes() {
		...
		this.attributes.forEach((attribute) => {
			...
			if (name === "slot") {
				...
				if (
					!(parent.type === "InlineComponent" || within_custom_element(parent))
				) {
					component.error(attribute, {
			...
			if (this.namespace !== namespaces.foreign) {
				...
				if (react_attributes.has(attribute.name)) {
					component.warn(attribute, {
						...
						message: `'${
							attribute.name
						}' is not a valid HTML attribute. Did you mean '${react_attributes.get(
							attribute.name
						)}'?`,
					});
				}
	...
	validate_special_cases() {
		...
		if (this.name === "a") {
			...
			if (href_attribute) {
				...
				if (
					href_value === "" ||
					href_value === "#" ||
					/^\W*javascript:/i.test(href_value)
				) {
				...
				const id_attribute_valid =
					id_attribute && id_attribute.get_static_value() !== "";
				const name_attribute_valid =
					name_attribute && name_attribute.get_static_value() !== "";
		...
		if (this.name === "label") {
			const has_input_child = this.children.some(
				(i) => i instanceof Element$1 && a11y_labelable.has(i.name)
			);
			...
			if (name === "value") {
				...
				if (type !== "file") {
					component.error(binding, {
						code: "invalid-binding",
						message:
							"'files' binding can only be used with <input type=\"file\">",
					});
				}
			} else if (name === "open") {
				...
				if (
					...
					component.error(binding, {
						...
						message: `'${
							binding.name
						}' is not a valid binding on <svg>. Use '${name.replace(
							"offset",
							"client"
						)}' instead`,
					});
				...
				if (!contenteditable) {
					component.error(binding, {
						...
						message:
							"'contenteditable' attribute is required for textContent and innerHTML two-way bindings",
					});
	...
	validate_content() {
		...
		if (
			this.bindings.some((binding) =>
				["textContent", "innerHTML"].includes(binding.name)
			)
		)
			return;
		...
		this.handlers.forEach((handler) => {
			...
			handler.modifiers.forEach((modifier) => {
				if (!valid_modifiers.has(modifier)) {
					component.error(handler, {
						code: "invalid-event-modifier",
						message: `Valid event modifiers are ${list(
							Array.from(valid_modifiers)
						)}`,
					});
				}
				...
				if (modifier === "passive") {
					if (passive_events.has(handler.name)) {
						...
						component.warn(handler, {
							code: "redundant-event-modifier",
							message:
								"The passive modifier only works with wheel and touch events",
						});
					}
				}
	...
	add_css_class() {
		...
		if (class_attribute && !class_attribute.is_true) {
			if (
				class_attribute.chunks.length === 1 &&
				class_attribute.chunks[0].type === "Text"
			) {
...
class InlineComponent$1 extends Node {
	...
	constructor(component, parent, scope, info) {
		super(component, parent, scope, info);
		InlineComponent$1.prototype.__init.call(this);
		InlineComponent$1.prototype.__init2.call(this);
		InlineComponent$1.prototype.__init3.call(this);
		InlineComponent$1.prototype.__init4.call(this);
...
class Slot$1 extends Element$1 {
	...
	constructor(component, parent, scope, info) {
		super(component, parent, scope, info);
		Slot$1.prototype.__init.call(this);
...
class Window extends Node {
	...
	constructor(component, parent, scope, info) {
		super(component, parent, scope, info);
		Window.prototype.__init.call(this);
		Window.prototype.__init2.call(this);
		Window.prototype.__init3.call(this);
		info.attributes.forEach((node) => {
			if (node.type === "EventHandler") {
				this.handlers.push(new EventHandler(component, this, scope, node));
...
function map_children(component, parent, scope, children) {
	...
	return children.map((child) => {
		...
		const use_ignores =
			child.type !== "Text" && child.type !== "Comment" && ignores.length;
...
class TemplateScope {
	...
	constructor(parent) {
		TemplateScope.prototype.__init.call(this);
		this.parent = parent;
...
class Component {
	...
	constructor(ast, source, name, compile_options, stats, warnings) {
		Component.prototype.__init.call(this);
		Component.prototype.__init2.call(this);
		Component.prototype.__init3.call(this);
		Component.prototype.__init4.call(this);
		Component.prototype.__init5.call(this);
		Component.prototype.__init6.call(this);
		Component.prototype.__init7.call(this);
		Component.prototype.__init8.call(this);
		Component.prototype.__init9.call(this);
		Component.prototype.__init10.call(this);
		Component.prototype.__init11.call(this);
		Component.prototype.__init12.call(this);
		Component.prototype.__init13.call(this);
		Component.prototype.__init14.call(this);
		Component.prototype.__init15.call(this);
		Component.prototype.__init16.call(this);
		Component.prototype.__init17.call(this);
		Component.prototype.__init18.call(this);
		Component.prototype.__init19.call(this);
		Component.prototype.__init20.call(this);
		Component.prototype.__init21.call(this);
								...
								extract_names(declarator.id).forEach((name) => {
									...
									if (variable.export_name) {
										...
										component.error(declarator, {
											...
											message:
												"Cannot declare props in destructured declaration",
										});
									}
			...
			if (attribute.type === "Attribute") {
				...
				switch (name) {
					...
					case "namespace": {
						...
						const message =
							"The 'namespace' attribute must be a string literal representing a valid namespace";
...
async function replace_in_code(regex, get_replacement, location) {
	const replacements = await calculate_replacements(
		regex,
		get_replacement,
		location.source
	);
...
class PreprocessResult {
	...
	constructor(source, filename) {
		this.source = source;
		this.filename = filename;
		PreprocessResult.prototype.__init.call(this);
		PreprocessResult.prototype.__init2.call(this);
...
function processed_tag_to_code(processed, tag_name, attributes, source) {
	...
	const tag_close_code = build_mapped_code(
		tag_close,
		tag_open.length + source.source.length
	);
...
function parse_tag_attributes(str) {
	...
	return str
		...
		.reduce((attrs, attr) => {
			...
			const [key, value] =
				i > 0 ? [attr.slice(0, i), attr.slice(i + 1)] : [attr];
			const [, unquoted] = (value && value.match(/^['"](.*)['"]$/)) || [];

			return {
				...attrs,
				[key]: _nullishCoalesce(
					_nullishCoalesce(unquoted, () => value),
					() => true
				),
			};
		}, {});
...
async function process_tag(tag_name, preprocessor, source) {
	...
	async function process_single_tag(
		...
		const no_change = () =>
			MappedCode.from_source(
				slice_source(tag_with_content, tag_offset, source)
			);
	...
	const { string, map } = await replace_in_code(
		tag_regex,
		process_single_tag,
		source
	);
...
export { VERSION, compile, parse$2 as parse, preprocess, walk };
