...
async function replace_async(file_basename, source, get_location, re, func) {
  ...
  for (const { offset, length, replacement } of await Promise.all(
    ...
    const content = StringWithSourcemap.from_source(
      file_basename,
      source.slice(last_end, offset),
      get_location(last_end)
    );
...
function get_replacement(
  ...
  const suffix_with_map = StringWithSourcemap.from_source(
    file_basename,
    suffix,
    get_location(offset + prefix.length + original.length)
  );
  ...
  if (processed.map) {
    ...
    if (source_index !== -1) {
      sourcemap_add_offset(
        decoded_map,
        get_location(offset + prefix.length),
        source_index
      );
    }
  }
  const processed_with_map = StringWithSourcemap.from_processed(
    processed.code,
    decoded_map
  );
  ...
  async function preprocess_tag_content(tag_name, preprocessor) {
    ...
    const res = await replace_async(
      ...
      async (match, attributes = "", content = "", offset) => {
        const no_change = () =>
          StringWithSourcemap.from_source(
            file_basename,
            match,
            get_location(offset)
          );
